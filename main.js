selectLv = prompt('selecione o nivel');
  ...
  var actual_level = selectLv || 1; //STARTING LEVEL
  widthScreem = window.innerWidth;
  ...
  var width = 0;  // find the largest row and use that as the game width
...
function showWin() {
 // cancelAnimationFrame(rAF);
  ...
  context.fillText('YOU WIN MDFK!', canvas.width / 2, canvas.height / 2);
  time++
...
function nextLevel() {
    ...
    drawBlocks= (marg, padding, blockWidth, row,  index)=>{
      
      margin = (widthScreem-marg)/2;
      ...
      context.fillRect(i*(blockWidth+padding)+margin, row*(blockWidth+padding)+margin, blockWidth, blockWidth);
      context.strokeRect(i*(blockWidth+padding)+margin-0.5, row*(blockWidth+padding)+margin-0.5, blockWidth-0.5, blockWidth-0.5);
            console.log(row*(blockWidth+padding)+margin);

      // X
      context.beginPath();
      context.moveTo(i*(blockWidth+padding)+margin-0.5, row*(blockWidth+padding)+margin-0.5);
      context.lineTo(i*(blockWidth+padding)+margin+blockWidth-0.5, row*(blockWidth+padding)+margin+blockWidth-0.5);
      context.moveTo(i*(blockWidth+padding)+margin+blockWidth-0.5, row*(blockWidth+padding)+margin-0.5);
      context.lineTo(i*(blockWidth+padding)+margin-0.5, row*(blockWidth+padding)+blockWidth+margin-0.5);
      context.stroke();
      }
    }

function structureBloks(innerWidth) {
  var blockWidth = 70;
  let padding = 0;
  margin = 0;
  var rows = 0;
  ...
  function ctxCtrl () {
    var dif = 0;
    for (let i = 0; i < levelsCont; i++) {
      ctxWidth = (blockWidth+padding)*i-padding;
      
      if (ctxWidth > innerWidth) {
        padding = padding-((ctxWidth-innerWidth)/(i-1));
        
        i = 0
      }else if (innerWidth == ctxWidth) {
        rows++;
        
        drawBlocks(innerWidth, padding, blockWidth, rows, i);
        
        if (i != levelsCont) {
          levelsCont -= i;
          i = 0;
        }
      }else if (ctxWidth < innerWidth && i == levelsCont-1) {
        padding = padding+((innerWidth-ctxWidth)/(i-1));
        i = 0;
    }
    }
...
function loop() {
  ...
  */
  ...
  structureBloks(widthScreem*(1-0.25));
}
}
